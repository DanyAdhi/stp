<div class="card shadow">
  ...
  <div class="card-body w-100">
    <form class="col-md-8" enctype="multipart/form-data" method="POST" action="<?=base_url('admin/jadwal/store')?>">
      <div class="form-group ">
        <label class="my-1 font-weight-bold">Program</label>
        <select class="custom-select my-1 mr-sm-2" name="program_id" id="program" required>
      ...
      <div class="form-group">
        ...
        <select class="custom-select my-1 mr-sm-2" name="days_id">
          <?php foreach ($days as $value): ?>
            <option value="<?=$value->id?>" <?=set_select('days_id', $value->id)?>>
              <?= $value->name ?>
            </option>
          <?php endforeach; ?>
        </select>
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" placeholder="08:00" name="start_time" value="<?=set_value('start_time')?>" data-timepicker required>
        <?=form_error('start_time', "<small class='form-text text-danger'>",'</small>') ?>
      </div>
      <div class="form-group">
        <label class="my-1 font-weight-bold">Jam Selesai</label>
        <input type="text" class="form-control" placeholder="13:00" name="end_time" value="<?=set_value('end_time')?>" data-timepicker required>
        <?=form_error('end_time', "<small class='form-text text-danger'>",'</small>') ?>
      </div>
      <div class="form-group ">
        <label class="my-1 font-weight-bold">Pembimbing</label>
        <select class="custom-select my-1 mr-sm-2" name="mentor_id" id="optionMentor" disabled>
        </select>
      </div>
