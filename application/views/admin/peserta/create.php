<div class="card shadow">
  <div class="card-header py-3 d-flex">
      <div>
        <h6 class=" font-weight-bold text-primary">Tambah Peserta</h6>
      </div>
  </div>
  <div class="card-body w-100">
    <form class="col-md-8" enctype="multipart/form-data" method="POST" action="<?=base_url('admin/peserta/store')?>">
      <div class="form-group">
        <label class="my-1 font-weight-bold">NIK</label>
        <input type="text" class="form-control" placeholder="Masukkan NIK" onkeypress="return inputNumber(event)" name="nik" value="<?=set_value('nik')?>" required>
      ...
      <div class="form-group ">
        ...
        <select class="custom-select my-1 mr-sm-2" name="religion">
          <?php foreach ($religions as $religion): ?>
            <option value="<?=$religion['name']?>" <?=set_select('religion', $religion['name'])?>>
              <?= $religion['name'] ?>
            </option>
          <?php endforeach; ?>
        </select>
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" placeholder="Masukkan nama Ayah atau Ibu" name="parents_name" value="<?=set_value('parents_name') ?>" required>
            ...
            <div class="form-group">
              <select class="selectpicker form-control" data-style="btn-white text-dark bordered border-secondary" data-live-search="true" title="---Pilih---" name="domicile" required>
                <?php foreach ($domicile as $data): ?>
                  <option <?=set_select('domicile',$data->name)?> > <?= $data->name ?> </option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
      ...
      <div class="form-group">
        ...
        <div class="custom-file">
          <input type="file" class="custom-file-input" name="ktp" accept="image/*" required>
      ...
      <div class="form-group">
        ...
        <div class="custom-file">
          <input type="file" class="custom-file-input" name="kk" accept="image/*" required>
          <label class="custom-file-label">Pilih gambar...</label>
        </div>
      </div>
      <div class="form-group">
        ...
        <div class="custom-file">
          <input type="file" class="custom-file-input" name="medical_certificate" accept="image/*" required>
          <label class="custom-file-label">Pilih gambar...</label>
        </div>
      </div>
